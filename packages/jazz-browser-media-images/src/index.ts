...
export async function createImage(
  ...
  const imageArrBuff = await imageBlobOrFile.arrayBuffer();
  const imageType = imageBlobOrFile.type;
  const image = await Image.load(imageArrBuff);
  const originalWidth = image.width;
  const originalHeight = image.height;

  const owner = options?.owner;

  const placeholderDataURL = image.resize({ width: 8 }).toDataURL("image/png");
  ...
  const fillImageResolutions = async () => {
    ...
    for (let resolution of imageResolutions.sort()) {
      ...
      const resizedImageBlob = await resizedImage.toBlob(imageType);
      const binaryStream = await FileStream.createFromBlob(resizedImageBlob, owner);
