...
- Access the latest edit directly with `_edits.fieldName` (not `.latest`)
- Cache historical queries if showing them in UI
- Be specific about time ranges you care about
- Remember that accessing history requires loading the CoValue
- If you find yourself using `_raw` often, reach out to us. We're always looking for ways to improve the API.